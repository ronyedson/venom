...
var HostLayer = /** @class */ (function () {
    ...
    HostLayer.prototype.spin = function (text, status) {
        var name = "session-".concat(this.session);
        ...
        var fullText = "[instance: ".concat(this.session);
        // if (this.spinStatus.state) {
        //   fullText += `, ${this.spinStatus.state}`;
        // }
        fullText += "]: ".concat(text);
        var prevText = '';
        try {
                ...
                switch (_a.label) {
                    ...
                    case 5:
                        if (this.options.logQR) {
                            ...
                            this.spin("Waiting for QRCode Scan: Attempt ".concat(attempt));
                        }
                        if (catchQR) {
                ...
                switch (_a.label) {
                    ...
                    case 2:
                        ...
                        if (typeof authenticated === 'object' && authenticated.type) {
                            this.spin("Error http: ".concat(authenticated.type), 'fail');
                            this.page.close();
                            throw "Error http: ".concat(authenticated.type);
                        }
                        this.startAutoClose();
